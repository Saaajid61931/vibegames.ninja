...
Antigravity 2.0 has a built-in `/goal` command. This skill intentionally uses `goal-keeper` so it can coexist with the built-in command while still giving you a reusable goal-state workflow.
...
4. Note constraints from the user, repository, tools, permissions, and current project.
