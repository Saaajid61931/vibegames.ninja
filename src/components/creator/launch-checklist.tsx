  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-primary-text">Launch Checklist</p>
        ...
        {checklist.items.map((item) => (
          <div key={item.id} className="rounded-lg border border-border bg-canvas p-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className={`text-sm font-medium ${item.done ? "text-text" : "text-text-secondary"}`}>
                  {item.label}
                </p>
