...
interface EditGamePageClientProps {
  gameId: string
}

export function EditGamePageClient({ gameId }: EditGamePageClientProps) {
  ...
  const fetchGame = useCallback(async () => {
    try {
      ...
      setFormData({
        ...
        tags: data.tags || "",
        aiTool:
          data.aiTool && AI_TOOLS.some((tool) => tool.value === data.aiTool)
            ? data.aiTool
            : "",
        aiModel:
          data.aiModel && AI_MODELS.some((model) => model.value === data.aiModel.toLowerCase())
            ? data.aiModel.toLowerCase()
            : "",
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                >
                  ...
                  {thumbnailPreview || thumbnailFile ? (
                    ...
                    <div className="flex flex-col items-center gap-3">
                      <Image
                        src={game.thumbnail}
                        alt={`Current thumbnail for ${game.title}`}
                        width={512}
                        height={288}
                        unoptimized
            ...
            <Card variant="arcade">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label variant="arcade">AI Tool Used</Label>
                    <Select
                    ...
                    >
                      ...
                      <SelectContent variant="arcade">
                        {AI_TOOLS.map((tool) => (
                          <SelectItem variant="arcade" key={tool.value} value={tool.value}>
                            {tool.label}
                          </SelectItem>
                        ))}
                      </SelectContent>
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <SelectContent variant="arcade">
                        {AI_MODELS.map((model) => (
                          <SelectItem variant="arcade" key={model.value} value={model.value}>
                            {model.label}
                          </SelectItem>
                        ))}
                      </SelectContent>
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
              <Link href="/creator">
                <Button type="button" variant="arcade-outline" className="w-full sm:w-auto font-arcade">
                  Cancel
                </Button>
              </Link>
