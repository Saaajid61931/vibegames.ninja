...
export function MobileReelsFeed({
  ...
  const [activeIndex, setActiveIndex] = useState(showIntro ? -1 : 0)
  ...
  const syncActiveSlide = useCallback(() => {
    ...
    const nextIndex = Math.min(Math.max(snappedPosition - (showIntro ? 1 : 0), showIntro ? -1 : 0), maxIndex)
    setActiveIndex((current) => current === nextIndex ? current : nextIndex)
  }, [feedGames.length, showIntro])
  ...
  return (
    <div className="relative flex h-[100dvh] w-full flex-col overflow-hidden bg-canvas text-white">
      ...
      >
        {showIntro && <MobileHomeIntroSlide
