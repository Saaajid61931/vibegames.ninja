    ...
    >
      <DropdownMenuTrigger asChild>
        <Button
          variant="outline"
          size="icon"
          className="relative border-border bg-surface"
          aria-label={buttonLabel}
        >
          <Bell className="h-4 w-4 text-primary-text" />
          {displayedUnreadCount > 0 ? (
            <span className="absolute -right-1 -top-1 inline-flex min-w-5 items-center justify-center rounded-full bg-arcade-red px-1.5 py-0.5 text-xs font-bold text-white">
              {unreadLabel}
            </span>
          ) : null}
        </Button>
      ...
      <DropdownMenuContent align="end" className="w-[360px] p-0">
        <div className="flex items-center justify-between px-4 py-3">
          <div>
            ...
            <p className="text-xs text-text-tertiary">Latest activity across your games and profile.</p>
          </div>
        ...
        {notifications.length === 0 ? (
          <div className="px-4 py-6 text-center">
            ...
            <p className="mt-1 text-xs text-text-tertiary">Follows, comments, ratings, and structured feedback will show up here.</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-start gap-3">
                    <span
                      className={`mt-1 h-2 w-2 flex-shrink-0 rounded-full ${
                        notification.read ? "bg-border-strong" : "bg-arcade-red"
                      }`}
                    >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-3">
                        <p className="text-sm font-medium leading-5 text-text">{notification.title}</p>
